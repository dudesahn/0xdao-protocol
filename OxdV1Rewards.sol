...
// Part: IOxLens (Here because pragma 0.5.17)
interface IOxLens {
...
// Part: IPartnersRewards (Here because pragma 0.5.17)
interface IPartnersRewards {
...
// Part: IOxdV1Redeem (Here because pragma 0.5.17)
...
contract OxdV1Rewards is ReentrancyGuard, Pausable, ProxyImplementation {
    using SafeMath for uint256;
    ...
    IPartnersRewards internal partnersRewardsPool;
    IERC20 internal oxdV1;
    IERC20 internal oxSolid;
    IERC20 internal solid;
    IOxdV1Redeem internal oxdV1Redeem;
    ...
    uint256 public oxdV1veSOLID = 2376588000000000000000000;
    uint256 public basis = 10000;
    uint256 public stakingCapMultiplier = 10000;
    ...
    function initialize(
        address _oxLens,
        address _oxdV1,
        IOxdV1Redeem _oxdV1Redeem
    ) public {
        require(address(oxLens) == address(0), "Already initialized");
        oxLens = IOxLens(_oxLens);
        partnersRewardsPool = IPartnersRewards(
            oxLens.partnersRewardsPoolAddress()
        );
        oxdV1 = IERC20(_oxdV1);
        oxSolid = IERC20(oxLens.oxSolidAddress());
        solid = IERC20(oxLens.solidAddress());
        oxdV1Address = _oxdV1;
        partnersRewardsPoolAddress = oxLens.partnersRewardsPoolAddress();
        oxdV1RedeemAddress = address(_oxdV1Redeem);
        oxdV1Redeem = _oxdV1Redeem;
    }
    ...
    function stakingCap(address account) public view returns (uint256 cap) {
        uint256 _burnt = oxdV1Burnt(account);
        cap = _burnt
            .mul(oxdV1veSOLID)
            .mul(oxLens.solidInflationSinceInception())
            .mul(stakingCapMultiplier)
            .div(oxdV1SnapshotTotalSupply)
            .div(1e18)
            .div(basis); //inflation is 1e18 based //stakingCapMultiplier is basis based
    }

    function oxdV1Burnt(address account) public view returns (uint256) {
        return oxdV1Redeem.oxdV1Burnt(account);
    }
        ...
        updateReward(account)
    {
        ...
        uint256 _newBalance = _balances[account].add(amount);
        require(
            _newBalance <= stakingCap(account) ||
                msg.sender == oxdV1RedeemAddress,
            "Exceeds staking cap."
        );
        stakingToken.safeTransferFrom(msg.sender, address(this), amount);

        //stake into partnersRewardsPoolAddress
        stakingToken.approve(partnersRewardsPoolAddress, amount);
        partnersRewardsPool.stake(amount);

        _totalSupply = _totalSupply.add(amount);
        _balances[account] = _newBalance;
        ...
        updateReward(msg.sender)
    {
        ...
        partnersRewardsPool.withdraw(amount);
    ...
    /**
     * @notice Sets stakingCapMultiplier
     * @param _stakingCapMultiplier desired amount for the multiplier used to cap oxSOLID staking amount based on oxdV1Burnt
     */
    function setStakingCapMultiplier(uint256 _stakingCapMultiplier)
        external
        onlyOwner
    {
        stakingCapMultiplier = _stakingCapMultiplier;
    }
    ...
    modifier updateReward(address account) {
        uint256 _rewardTokensLength = partnersRewardsPool.rewardTokensLength();
        address _rewardToken;
        uint256 _earned;
        {
            for (uint256 i; i < _rewardTokensLength; i++) {
                _rewardToken = partnersRewardsPool.rewardTokens(i);
                _earned = partnersRewardsPool.earned(
                    address(this),
                    partnersRewardsPool.rewardTokens(i)
                );
                //going to rely on the permissioned nature of bribes and hope earned amount is what actually going to be transfered here. Will change it to a more robust model if there's time (balanceAfter - balanceBefore)
                if (rewardData[_rewardToken].rewardsDuration == 0) {
                    addReward(_rewardToken, address(this), 86400 * 7);
                }
                notifyRewardAmount(_rewardToken, _earned);
            }
        }

        partnersRewardsPool.getReward();
